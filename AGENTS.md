...
- **改 CSS/JS 不再需要升 `CACHE_VERSION`**：`/static/css/`、`/static/js/` 走 network-first，下次加载就是新的，断网才回退缓存；cache-first 只留给体积稳定的图标/manifest；`/media/` 直连网络不缓存（用户上传同名覆盖后不该看到旧文件）。现在只有改 precache 清单才升版本号。另外 `base.html` 里清理旧窄作用域注册的迁移代码必须枚举后按 `scope` 筛（`getRegistrations()` + `reg.scope.indexOf('/static/') !== -1`）；**禁止 `getRegistration('/static/').unregister()`**，那个 API 返回的是「覆盖给定 URL 的最长作用域注册」，遗留注册清干净后它就是根注册本身，等于每次开页自删根注册（实测因此整条离线兜底失效）
...
- **Daily 页桌面两列**：`daily.html` 用 `.daily-cols` / `.daily-main` / `.daily-rail` 复用详情页同一套声明（CSS 里是 `.detail-cols, .daily-cols {…}` 分组选择器，**不要拆成两份拷贝**，否则必然漂移；也因此 `css.index('.detail-cols {')` 这类裸字符串查找会失效，测试用 `activities/tests.py::_css_rules`）。与详情页的差别只在**右列整块 DOM 排在左列之前**（今日概览：打卡与提醒·关键数字·今日进度·今日摘要·本周消费），桌面端靠 `.daily-cols .daily-rail { order: 2 }` 换到右侧 —— `order` 只在网格内生效、媒体查询外不定义，因此**移动端阅读顺序与改造前逐块一致**，无需调整块顺序。sticky 列有高度预算（`max-height: calc(100vh - 5.5rem)`）：往 `.daily-rail` 里加卡片要算总高，超了会出现列内滚动并裁掉底部卡片（靠压缩内容解决，别改 `max-height`）；rail 顶层子块间距统一 `md:mb-6`。三个次要长列表（今日进行中 / 即将开始 / 最近完成）**桌面端默认折叠**，靠 `window.matchMedia('(min-width: 768px)')` 门控默认值，移动端仍默认展开 —— 新增折叠分区要同时进 restore 脚本的 `sections` 数组，且禁止另写一套折叠逻辑（复用 `toggleSection()` + `localStorage['daily_section_'+id]`）。「今日进度」卡上的习惯完成率靠 `htmx:afterSwap` 监听 + `[data-habit-row]` 重算，改打卡表单的 `hx-swap` 会静默失效。改这个模板的块顺序/显隐要同步 `DailyDesktopLayoutTest` 的顺序锁与进度卡锁。
